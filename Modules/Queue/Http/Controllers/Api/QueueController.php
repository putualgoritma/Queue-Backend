...
{
    public function testAudio(Request $request, QueueService $service)
    {
        $data = $service->testAudio($request);
        if (!$data->success) {
            return sendError($data->message, '', '404');
        }
        return sendResponse($data->data, 'Test Audio successfully.');
    }
    
    public function redisplay(Request $request, QueueService $service)
    {
        $row = $service->redisplay($request);
        if (!$row->success) {
    ...
    public function index(Request $request, QueueService $service)
    {
        ...
        return sendResponse($data->data, 'User list successfully.');
    }
    ...
    public function selected(Request $request, QueueService $service)
    {
        $data = $service->selected($request);
        if (!$data->success) {
            return sendError($data->message, '', '404');
        }
        return $data;
